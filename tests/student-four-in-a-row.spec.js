...
const FAST_VISUALS = { theme:'canyon-classic', pieces:'marble', animateDrops:false, thinkingPause:false, reducedMotion:true, focusBoard:false };
...
async function setup(page, mode = 'local', human = 1, level = 'learning') {
  ...
  if (mode === 'computer') {
    await page.locator('#humanSelect').selectOption(String(human));
...
async function seed(page, moves, extra = {}) {
  await page.evaluate(({ key, game }) => localStorage.setItem(key, JSON.stringify({ revision:'fixture', savedAt:1, game })), {
    key: KEY,
    game: { version:1, moves, ...options, ...extra },
  });
  await page.reload();
}
...
test.beforeEach(async ({ page, context, baseURL }) => {
  ...
  await context.route('**/*', route => {
    ...
    if (url.pathname.startsWith('/.netlify/functions/')) {
      return route.fulfill({ status:503, contentType:'application/json', body:'{"ok":false,"error":"Synthetic Four in a Row test"}' });
    }
  ...
  }, { code:CODE, visualKey:VISUAL_KEY, visual:FAST_VISUALS });
  await page.reload();
...
test('two-player games win, highlight four, stop accepting moves, and resume the result', async ({ page }) => {
  ...
  await page.locator('[data-column="4"]').press('Enter');
  expect((await saved(page)).game.moves).toHaveLength(7);
  await page.reload();
  await expect(page.locator('.cell.winner')).toHaveCount(4);
  await page.locator('#undoBtn').click();
  await expect(page.locator('.cell.winner')).toHaveCount(0);
...
test('autosave restores a two-player game and an undone move after reload', async ({ page }) => {
  await setup(page);
  for (const column of [0,1,2]) await drop(page,column);
  await page.reload();
  await expect(page.locator('#moveCount')).toHaveText('3 of 42 spaces');
  await page.locator('#undoBtn').click();
  await page.reload();
...
test('coaching explains a block and can be switched off without making a move', async ({ page }) => {
  ...
  expect((await saved(page)).game.moves).toHaveLength(6);
  await page.reload();
  await expect(page.locator('#coachToggle')).not.toBeChecked();
});
...
test('challenges give useful feedback, accept alternate answers, and preserve the saved game', async ({ page }) => {
  ...
  await page.locator('#exerciseHintBtn').click();
  await page.locator('#exerciseHintBtn').click();
  await page.locator('#exerciseHintBtn').click();
...
test('guided lessons require a correct move and keep practice separate from saves', async ({ page }) => {
  ...
  await page.locator('[data-exercise="first-drop"]').click();
  await drop(page,0);
  await expect(page.locator('#exerciseSuccess')).toBeHidden();
  await drop(page,3);
  await expect(page.locator('#exerciseSuccess')).toBeVisible();
  ...
  await expect(page.locator('#exerciseTitle')).toHaveText('Connect across');
});

test('HD board themes and material-only piece styles persist without player labels on pieces', async ({ page }) => {
  await setup(page);
  await drop(page,0);
  await drop(page,1);
  ...
  for (const pieces of ['marble','wood','metal','canyon-stone','high-contrast']) {
    await page.locator('#piecesSelect').selectOption(pieces);
    await expect(page.locator('body')).toHaveAttribute('data-pieces',pieces);
  }
  await expect(page.locator('[data-cell="35"] > .piece-surface')).toHaveText('');
  await expect(page.locator('[data-cell="36"] > .piece-surface')).toHaveText('');
  ...
  await expect(page.locator('body')).toHaveAttribute('data-pieces','high-contrast');
});
...
test('keyboard column navigation and number shortcuts each make one move', async ({ page }) => {
  ...
  await expect(page.locator('[data-column="2"]')).toBeFocused();
  await page.keyboard.press('Enter');
  expect((await saved(page)).game.moves).toEqual([2]);
  await page.keyboard.press('7');
  expect((await saved(page)).game.moves).toEqual([2,6]);
  await page.keyboard.press('Home');
  await expect(page.locator('[data-column="0"]')).toBeFocused();
  await page.keyboard.press('Space');
  expect((await saved(page)).game.moves).toEqual([2,6,0]);
...
test('mobile and Chromebook layouts keep the board visible without horizontal overflow', async ({ page }) => {
  for (const viewport of [{width:375,height:812},{width:1366,height:768}]) {
    ...
    const box = await page.locator('.board-card').boundingBox();
  ...
  const board = page.frameLocator('#contentIframe').locator('#board');
  await expect(board).toBeVisible();
  expect(await board.evaluate(element => element.getBoundingClientRect().right <= innerWidth + 1)).toBe(true);
});
...
test('a quota failure keeps the board and prior save and visibly reports the unsaved move', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    Storage.prototype.setItem = function(key,value) {
      if (key.startsWith('rc_four_v1:')) throw new Error('Test quota exceeded');
      return original.call(this,key,value);
    };
  });
  await drop(page,0);
  await expect(page.locator('#moveCount')).toHaveText('1 of 42 spaces');
  await expect(page.locator('#storageStatus')).toContainText('Test quota exceeded');
  expect(await saved(page)).toEqual(before);
});
...
test('unreadable saves are kept until explicitly replaced', async ({ page }) => {
  ...
  await page.locator('#newGameBtn').click();
  await expect(page.locator('#startGameBtn')).toHaveText('Replace saved game & start');
  await page.locator('[data-close="newGameDialog"]').click();
  expect(await page.evaluate(key => localStorage.getItem(key),KEY)).toBe('{broken');
  await setup(page);
  expect((await saved(page)).game.moves).toEqual([]);
});
...
test('stale tabs cannot silently replace the newer game', async ({ page }) => {
  ...
  await page.evaluate(key => {
    ...
    data.revision='newer-tab'; data.game.moves=[0,1];
    localStorage.setItem(key,JSON.stringify(data));
  },KEY);
  await drop(page,2);
  await expect(page.locator('#storageStatus')).toContainText('Another tab changed');
  expect((await saved(page)).game.moves).toEqual([0,1]);
  await openCode(page,'#copyBtn');
  expect(JSON.parse(await page.locator('#gameCode').inputValue()).moves).toEqual([0,2]);
});

test('changing students locks the old game and keeps the next student’s save separate', async ({ page }) => {
  await setup(page);
  await drop(page,0);
  await page.evaluate(() => sessionStorage.setItem('rc_user_code','FOUR_TEST_B'));
  await drop(page,2);
  ...
  await page.reload();
  await expect(page.locator('#moveCount')).toHaveText('New game');
...
test('game-code import validates first and confirms replacement before saving', async ({ page }) => {
  ...
  await page.locator('#gameCode').fill('{broken');
  await page.locator('#transferAction').click();
  await expect(page.locator('#transferError')).toContainText('complete Four in a Row');
  expect(await saved(page)).toEqual(before);
  const code = JSON.stringify({version:1,moves:[3,2,4],...options});
  await page.locator('#gameCode').fill(code);
  await page.locator('#transferAction').click();
  await expect(page.locator('#startGameBtn')).toHaveText('Replace saved game & load');
  expect(await saved(page)).toEqual(before);
  await page.locator('#startGameBtn').click();
  expect((await saved(page)).game.moves).toEqual([3,2,4]);
  await openCode(page,'#copyBtn');
  expect(await page.locator('#gameCode').inputValue()).not.toContain(CODE);
});

test('leaving or undoing a delayed computer turn cancels the late reply', async ({ page }) => {
  await page.route('**/activities/four-in-a-row/worker.js*', route => route.fulfill({
    contentType:'text/javascript',
    body:'self.onmessage=e=>setTimeout(()=>self.postMessage({id:e.data.id,column:2}),600);',
  }));
  await setup(page,'computer');
  await drop(page,0);
  await page.locator('[data-view="learn"]').click();
...
test('a malformed computer reply pauses safely and a retry can continue', async ({ page }) => {
  ...
  await page.route('**/activities/four-in-a-row/worker.js*', route => route.fulfill({
    contentType:'text/javascript',
    body:++attempts === 1 ? 'self.onmessage=()=>self.postMessage(null);' : 'self.onmessage=e=>self.postMessage({id:e.data.id,column:2});',
  }));
  await setup(page,'computer');
  await drop(page,0);
...
test('the real worker rejects malformed requests and processes a later valid request', async ({ page }) => {
  const results = await page.evaluate(async () => {
    const worker = new Worker('/activities/four-in-a-row/worker.js?v=20260908-four-hd-1');
    const send = data => new Promise((resolve,reject) => {
      worker.onmessage=e=>resolve(e.data); worker.onerror=e=>reject(new Error(e.message)); worker.postMessage(data);
    });
  ...
  expect(results[2].column).toBeGreaterThanOrEqual(0);
  expect(results[2].column).toBeLessThan(7);
});
...
test('older-browser fallbacks keep dialogs, keyboard focus, computer play, and imports usable', async ({ page }) => {
  ...
  await page.locator('#startGameBtn').focus();
  ...
  await expect(page.locator('#moveCount')).toHaveText('2 of 42 spaces');
  await page.locator('#settingsBtn').click();
  await page.locator('#themeSelect').selectOption('wood');
  await page.keyboard.press('Escape');
  ...
  await page.reload();
  await expect(page.locator('#moveCount')).toHaveText('3 of 42 spaces');
  await expect(page.locator('body')).toHaveAttribute('data-theme', 'wood');
  expect(errors).toEqual([]);
});

test('the board stays legible without dynamic viewport units or CSS aspect ratios', async ({ page }) => {
  for (const path of ['game.css','four-hd.css']) {
    await page.route(`**/activities/four-in-a-row/${path}*`, async route => {
  ...
  for (const viewport of [{width:375,height:812}, {width:1366,height:768}]) {
    await page.setViewportSize(viewport);
    const sizes = await page.locator('.cell').evaluateAll(cells => cells.map(cell => {
      const box=cell.getBoundingClientRect(); return {width:box.width,height:box.height};
    }));
    expect(sizes).toHaveLength(42);
    for (const size of sizes) {
      expect(size.width).toBeGreaterThan(20);
      expect(Math.abs(size.height-size.width)).toBeLessThan(3);
    }
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  }
  await drop(page, 0);
