...
test('finalized: tree/table/byUnit viewModes all round-trip correctly', () => {
  for (const mode of ['tree', 'table', 'byUnit']) {
    const storage = makeMockStorage();
    const ctx = makeFilterContext(storage);
    ctx.filters.finalized.viewMode = mode;
    ctx.saveFilters();
    const ctx2 = makeFilterContext(storage);
    ctx2.loadFilters();
    assert.strictEqual(ctx2.filters.finalized.viewMode, mode, `viewMode '${mode}' should round-trip`);
  }
});
...
test('finalized: selectedTags with non-string items → non-strings filtered out', () => {
  const storage = makeMockStorage();
  storage.setItem('rc_tc_library_filters_v1', JSON.stringify({
    finalized: { selectedTags: ['quiz', 42, null, 'vocab'] }
  }));
  const ctx = makeFilterContext(storage);
  ctx.loadFilters();
  assert.deepStrictEqual(ctx.filters.finalized.selectedTags, ['quiz', 'vocab']);
});
