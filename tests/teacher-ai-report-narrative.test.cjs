...
var handler = require('../netlify/functions/teacher-ai-report-narrative').handler;
...
function makeAnthropicResponse(narrative, status) {
  var s = status || 200;
...
test('success with audience "admin" returns 200 and narrative', async function() {
  var expectedNarrative = 'Student made strong progress in Reading this quarter, achieving 72% accuracy against a baseline of 60%.';
  global.fetch = makeAnthropicResponse(expectedNarrative);
  var res = await handler(authedEvent(validBody));
  assert.strictEqual(res.statusCode, 200);
  var parsed = JSON.parse(res.body);
  assert.strictEqual(parsed.ok, true);
  assert.strictEqual(parsed.narrative, expectedNarrative);
});

test('success with audience "parent" returns 200 and narrative', async function() {
  var parentBody = Object.assign({}, validBody, { audience: 'parent' });
  var expectedNarrative = 'Your child is working on reading skills and has improved from 60% to 72% this quarter!';
  global.fetch = makeAnthropicResponse(expectedNarrative);
