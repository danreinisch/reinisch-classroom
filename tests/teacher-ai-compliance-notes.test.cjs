// Unit tests for netlify/functions/teacher-ai-compliance-notes.js
// Tests teacher auth, input validation, Anthropic integration, error handling
// Run with: node tests/teacher-ai-compliance-notes.test.cjs
...
var handler = require('../netlify/functions/teacher-ai-compliance-notes').handler;
...
async function runAll() {
  console.log('Running teacher-ai-compliance-notes unit tests...\n');
  var failed = 0;
  ...
  console.log('\n\u2713 All teacher-ai-compliance-notes tests passed!');
}
...
var validBody = {
  ...
  goals: validGoals,
  complianceData: validComplianceData,
  quarterLabel: 'Q1 (Sep-Nov 2025)',
};

function makeAnthropicResponse(notes, status) {
  var s = status || 200;
...
test('success returns 200 and notes', async function() {
  var expectedNotes = '1. Progress Monitoring Statement\nProgress monitoring was conducted in accordance with the IEP schedule for Q1 (Sep-Nov 2025).\n\n2. Data Collection Summary\nA total of 8 data collection events were completed out of 9 expected, representing an 88.9% data collection rate.\n\n3. Service Delivery Confirmation\nAll IEP services were delivered as specified during the reporting period.\n\n4. Goal Progress Overview\nGoal G001 (Reading): Current value 72% against a target of 80%. Student is making adequate progress.\n\n5. Compliance Concerns\nNo significant compliance concerns were identified for this reporting period.';
  global.fetch = makeAnthropicResponse(expectedNotes);
  var res = await handler(authedEvent(validBody));
  assert.strictEqual(res.statusCode, 200);
  var parsed = JSON.parse(res.body);
  assert.strictEqual(parsed.ok, true);
  assert.strictEqual(parsed.notes, expectedNotes);
});

test('success without goals returns 200', async function() {
  var bodyNoGoals = Object.assign({}, validBody, { goals: [] });
  var expectedNotes = 'No active goals found for this reporting period.';
  global.fetch = makeAnthropicResponse(expectedNotes);
  var res = await handler(authedEvent(bodyNoGoals));
  assert.strictEqual(res.statusCode, 200);
  var parsed = JSON.parse(res.body);
  assert.strictEqual(parsed.ok, true);
  assert.strictEqual(parsed.notes, expectedNotes);
...
test('success without complianceData returns 200', async function() {
  var bodyNoCompliance = Object.assign({}, validBody, { complianceData: null });
  var expectedNotes = 'Compliance notes drafted without detailed compliance metrics.';
  global.fetch = makeAnthropicResponse(expectedNotes);
  var res = await handler(authedEvent(bodyNoCompliance));
  assert.strictEqual(res.statusCode, 200);
  var parsed = JSON.parse(res.body);
  assert.strictEqual(parsed.ok, true);
});
