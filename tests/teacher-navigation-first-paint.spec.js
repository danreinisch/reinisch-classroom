  ...
  test(`slow shell ${route || 'overview'}: sidebar is stable at first paint`, async ({ page }, info) => {
    ...
    try {
      ...
      if (!baseline) {
        ...
        await expect(page.locator('link[data-teacher-scene-preload]')).toHaveAttribute('href', '/assets/bg/moonlit-canyon.webp?v=20260907-moonlit1');
      }
      release();
...
if (!baseline) {
  ...
  test('pending canyon image never holds up authentication, sign-out or login', async ({ page }, info) => {
    ...
    let release;
    const held = new Promise(resolve => { release = resolve; });
    await page.route('**/assets/bg/moonlit-canyon.webp*', async route => { await held; await route.abort(); });
    try {
