...
async function isolate(page, { imageFailure = false, denied = false } = {}) {
  ...
  await page.route('**/*', route => {
    ...
    if (imageFailure && url.pathname === '/assets/bg/rc-annotated-canyon.svg') return route.abort();
...
async function assertDecodedScene(page) {
  ...
  await expect(page.locator('.tc-shell')).toHaveCSS('background-image', /rc-annotated-canyon\.svg/);
