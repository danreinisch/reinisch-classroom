// Unit tests for netlify/functions/teacher-ai-report-summary.js
// Tests teacher auth, input validation, Anthropic integration, error handling
// Run with: node tests/teacher-ai-report-summary.test.cjs
...
var handler = require('../netlify/functions/teacher-ai-report-summary').handler;
...
async function runAll() {
  console.log('Running teacher-ai-report-summary unit tests...\n');
  var failed = 0;
  ...
  console.log('\n\u2713 All teacher-ai-report-summary tests passed!');
}
...
var validBody = {
  ...
  goals: validGoals,
  quarterLabel: 'Q1 (Sep–Nov 2025)',
  assignmentSummary: validAssignmentSummary,
  audience: 'admin',
};

function makeAnthropicResponse(summary, status) {
  var s = status || 200;
...
test('returns 400 when quarterLabel is missing', async function() {
  var body = Object.assign({}, validBody, { quarterLabel: '' });
  var res = await handler(authedEvent(body));
  assert.strictEqual(res.statusCode, 400);
  var parsed = JSON.parse(res.body);
  assert.strictEqual(parsed.ok, false);
  assert.ok(parsed.error.includes('quarterLabel'));
});

test('returns 400 when audience is invalid', async function() {
  var body = Object.assign({}, validBody, { audience: 'invalid' });
  var res = await handler(authedEvent(body));
  assert.strictEqual(res.statusCode, 400);
  var parsed = JSON.parse(res.body);
  assert.strictEqual(parsed.ok, false);
  assert.ok(parsed.error.includes('audience'));
});
...
test('success with audience "admin" returns 200 and summary', async function() {
  var expectedSummary = 'Overall, Test Student demonstrated steady progress this quarter, achieving 72% in Reading against an 80% mastery target.\n\nGoal G001 shows an upward trend with 8 data points collected, indicating consistent engagement.\n\nThe IEP team should consider increasing the frequency of reading fluency checks and targeted tier-2 interventions to close the remaining gap to the 80% target.';
  global.fetch = makeAnthropicResponse(expectedSummary);
  var res = await handler(authedEvent(validBody));
  assert.strictEqual(res.statusCode, 200);
  var parsed = JSON.parse(res.body);
  assert.strictEqual(parsed.ok, true);
  assert.strictEqual(parsed.summary, expectedSummary);
});

test('success with audience "parent" returns 200 and summary', async function() {
  var parentBody = Object.assign({}, validBody, { audience: 'parent' });
  var expectedSummary = 'Your child has been working hard on reading skills this quarter and is making great progress toward their goals!\n\nIn the Reading goal area, they have shown an upward trend and are getting closer to the target each week.\n\nTo keep up this great momentum at home, try reading together for 15 minutes each evening and celebrating every small win along the way.';
  global.fetch = makeAnthropicResponse(expectedSummary);
  var res = await handler(authedEvent(parentBody));
  assert.strictEqual(res.statusCode, 200);
  var parsed = JSON.parse(res.body);
  assert.strictEqual(parsed.ok, true);
  assert.strictEqual(parsed.summary, expectedSummary);
...
test('success without assignmentSummary returns 200', async function() {
  var bodyNoAssignments = Object.assign({}, validBody, { assignmentSummary: null });
  var expectedSummary = 'Student is making progress.';
  global.fetch = makeAnthropicResponse(expectedSummary);
  var res = await handler(authedEvent(bodyNoAssignments));
  assert.strictEqual(res.statusCode, 200);
  var parsed = JSON.parse(res.body);
  assert.strictEqual(parsed.ok, true);
  assert.strictEqual(parsed.summary, expectedSummary);
