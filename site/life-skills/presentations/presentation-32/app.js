(function(){
  ...
  function init(){
    try{
      // --- status chips (helps debug Netlify “loads but doesn’t work”) ---
      const jsChip = document.getElementById("jsState");
      if(jsChip){
        jsChip.textContent = "JS: loaded";
        jsChip.classList.remove("warn","err");
        jsChip.classList.add("ok");
      }
      const voiceChip = document.getElementById("voiceState");
      if(voiceChip){
        const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
        if(SR){
          voiceChip.textContent = "Voice: available";
          voiceChip.classList.remove("warn","err");
          voiceChip.classList.add("ok");
        }else{
          voiceChip.textContent = "Voice: keyboard mic";
          voiceChip.classList.remove("ok","err");
          voiceChip.classList.add("warn");
        }
      }
