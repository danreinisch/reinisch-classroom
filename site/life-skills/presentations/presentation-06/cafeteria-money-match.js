...
(() => {
  ...
  window.__CAFETERIA_CASHIER_BUILD = 'v30-check-down-capture';
  ...
  function __attachReliableCheck(btn) {
    if (!btn || btn.dataset.rcCheckBound) return;
    btn.dataset.rcCheckBound = "1";
    ...
    const pointInside = (e) => {
      try {
        ...
        if (!r || !isFinite(r.left) || typeof x !== 'number' || typeof y !== 'number') return false;
    ...
    const fire = (e, src) => {
      ...
      __fireCheck(e, src || 'check');
    };

    // Fire on DOWN (most reliable — not dependent on click generation)
    const btnDown = (e) => fire(e, 'btn-down');
    const btnClick = (e) => fire(e, 'btn-click');

    btn.addEventListener('pointerdown', btnDown, { capture: true, passive: false });
    btn.addEventListener('mousedown', btnDown, true);
    btn.addEventListener('touchstart', btnDown, { capture: true, passive: false });
    btn.addEventListener('click', btnClick, true);

    // Document capture fallback: if something overlays the button, clicking its hitbox still fires.
    const docDown = (e) => {
      if (!btn.isConnected) return;
      // If the real target is the button, do it.
      try {
        if (e.target === btn || (e.target && e.target.closest && e.target.closest('#btnCheck'))) {
          return fire(e, 'doc-target');
        }
      } catch (_) {}
      // Otherwise, trust the geometry.
      if (pointInside(e)) return fire(e, 'doc-rect');
    };

    document.addEventListener('pointerdown', docDown, { capture: true, passive: false });
    document.addEventListener('mousedown', docDown, true);
    document.addEventListener('touchstart', docDown, { capture: true, passive: false });

    // Keyboard
    btn.addEventListener('keydown', (e) => {
      const k = e.key;
      if (k === 'Enter' || k === ' ') fire(e, 'key');
    }, true);
  }
