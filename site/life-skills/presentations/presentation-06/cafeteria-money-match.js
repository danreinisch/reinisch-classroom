...
(() => {
  ...
  window.__CAFETERIA_CASHIER_BUILD = 'v29-check-fix';
  ...
  function __attachReliableCheck(btn) {
    ...
    // Direct listeners (works when the button actually receives events)
    ['click','pointerup','mouseup','touchend'].forEach((t) => {
      btn.addEventListener(t, handler, { capture: true, passive: false });
    });

    // Document-level capture fallback (works when an overlay steals the target)
    document.addEventListener('pointerup', (e) => {
      ...
      if (!inside) return;
      const recentDrag = (Date.now() - __dragPulse) < 220;
      const targetIsBtn = e.target === btn || (e.target && e.target.closest && e.target.closest('#btnCheck'));
      if (recentDrag && !targetIsBtn) return;
      handler(e);
    }, true);
