...
(() => {
  window.__CAFETERIA_CASHIER_BUILD = 'v26-entry-check-reliable';
...
<svg viewBox="0 0 92 92" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${label} play coin">
  ...
  // ===== v26: Check reliability (capture + delegation) =====
  (function wireCheckReliable(){
    const btn = (typeof btnCheck !== 'undefined' && btnCheck) ? btnCheck : document.getElementById('btnCheck');
    if(!btn) return;
    ...
    const fire = (e) => {
      ...
      // visible micro-feedback so clicks don't feel "dead"
      try {
    ...
    const matches = (t) => {
      try { return t && t.closest && t.closest('#btnCheck'); } catch { return false; }
    };
    ...
    // Capture-phase delegation catches cases where bubbling is interrupted
    document.addEventListener('pointerup', handler, { capture: true });
    document.addEventListener('click', handler, { capture: true });

    // Direct listeners on the button too
