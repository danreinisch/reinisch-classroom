  ...
  // Build marker for debugging (helps confirm you are seeing the latest upload)
  window.__CAFETERIA_CASHIER_BUILD = "v25-check-hitbox";
  console.info("[cafeteria-money-match] v25-check-hitbox loaded");
...
(() => {
  ...
  function handleCheckPress(e){
    ...
    // Visual + console proof that the click was received (helps debugging in the iframe viewer).
    try{ console.log("[cafeteria-money-match] Check pressed"); }catch(_){ }
    try{
      const b = document.getElementById("btnCheck");
      if (b){
        b.classList.add("caf-pressed");
        const old = b.textContent;
        b.textContent = "Checking…";
        setTimeout(() => { b.classList.remove("caf-pressed"); b.textContent = old; }, 350);
      }
    }catch(_){ }
    ...
    try{
      ...
      try{ console.error("[cafeteria-money-match] check() threw", err); }catch(_){ }
      try{ setStatus("nope", "Check error — see console"); }catch(_){ }
    }
  }
...
<svg viewBox="0 0 92 92" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${label} play coin">
  ...
  // Ultra-reliable press handling for embedded/sandboxed viewers.
// We use CAPTURE phase + hit-testing so invisible overlays can’t eat the event.
function __cafHit(el, x, y) {
  if (!el) return false;
  const r = el.getBoundingClientRect();
  return x >= r.left && x <= r.right && y >= r.top && y <= r.bottom;
}

if (!window.__CAF_PRESS_DELEGATE__) {
  window.__CAF_PRESS_DELEGATE__ = true;

  function __cafIsCheckTarget(e) {
    const t = e.target;
    if (t && t.closest && t.closest("#btnCheck")) return true;
    if (typeof e.clientX === "number" && typeof e.clientY === "number") {
      return __cafHit(btnCheck, e.clientX, e.clientY);
    }
    return false;
  }

  function __cafIsUndoTarget(e) {
    const t = e.target;
    if (t && t.closest && t.closest("#btnUndo")) return true;
    if (typeof e.clientX === "number" && typeof e.clientY === "number") {
      return __cafHit(btnUndo, e.clientX, e.clientY);
    }
    return false;
  }

  const __cafDelegate = (e) => {
    try {
      if (__cafIsCheckTarget(e)) return handleCheckPress(e);
      if (__cafIsUndoTarget(e)) return undo(e);
    } catch (err) {
      console.error("[cafeteria-money-match] delegate crash:", err);
    }
  };

  // Use both document + window at capture phase.
  ["pointerup", "click"].forEach((type) => {
    document.addEventListener(type, __cafDelegate, true);
    window.addEventListener(type, __cafDelegate, true);
  });

  // Keyboard activation safety (Enter/Space)
  document.addEventListener("keydown", (e) => {
    if (e.key !== "Enter" && e.key !== " ") return;
    const ae = document.activeElement;
    if (!ae) return;
    if (ae.id === "btnCheck") return handleCheckPress(e);
    if (ae.id === "btnUndo") return undo(e);
  }, true);
}
