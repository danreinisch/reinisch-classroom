  ...
  window.__CAFETERIA_CASHIER_BUILD = "v18-csp-safe";
  try{ console.info("[CafeteriaCashier] JS loaded " + window.__CAFETERIA_CASHIER_BUILD); }catch(e){}

(() => {
  const storage = (() => {
    try {
      const t = "__rc_storage_test";
  ...
  function setSound(on){
    soundOn = !!on;
    storage.setItem("money_sound", soundOn ? "1" : "0");
  ...
  function setRole(nextRole){
    ...
    storage.setItem("money_role", role);
    updateModePill();
  }
  ...
  let targetCents = 0;
  // Role + modes
  let role = storage.getItem("money_role") || "customer"; // "customer" | "cashier"
  let soundOn = (storage.getItem("money_sound") || "1") === "1";
  ...
  let streak = Number(storage.getItem("money_streak") || "0");
  let best = Number(storage.getItem("money_best") || "0");
  let points = Number(storage.getItem("money_points") || "0");
...
<svg viewBox="0 0 92 92" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${label} play coin">
  ...
  function check(){
    ...
    if (running === goal){
      ...
      storage.setItem("money_streak", String(streak));
      storage.setItem("money_best", String(best));
      ...
      streak = 0;
      storage.setItem("money_streak", "0");
