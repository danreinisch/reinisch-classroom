import { bank, checkEdit } from "./sentence-workshop-content.js?v=20260906-sw3";
...
export function createSession(lessonId = "boundaries") {
  if (!["boundaries", "endings"].includes(lessonId)) throw new Error("Unknown workshop lesson");
  return {
    lessonId,
    phase: "intro",
    item: null,
...
function selectNext(session, phase) {
  ...
  const next =
    session.lessonId === "endings" && phase === "simpler"
      ? items.find((item) => item.intent === session.item?.intent && !session.records[item.id]) ||
