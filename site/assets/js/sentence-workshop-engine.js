import { bank, checkEdit } from "./sentence-workshop-content.js?v=20260907-sw9";
...
function lessonFor(session) {
  if (session.lessonId === "joining")
    return { bank: joining.bank, check: joining.checkEdit, guided: 4, applied: 4 };
...
export function createSession(lessonId = "boundaries") {
  if (!["boundaries", "endings", "repairs", "commas", "openings", "joining"].includes(lessonId))
...
function selectNext(session, phase) {
  ...
  const next =
    session.lessonId !== "boundaries" && phase === "simpler"
      ? items.find(
          (item) =>
            (["repairs", "commas", "openings", "joining"].includes(session.lessonId)
...
export function submit(session, edit) {
  ...
  const key =
    session.lessonId === "joining"
      ? JSON.stringify([edit?.link, edit?.comma])
      : ["commas", "openings"].includes(session.lessonId)
        ? JSON.stringify(
...
export function summary(session) {
  ...
  return {
    ...
    ...(session.lessonId === "joining"
      ? {
          freshConnections: checks.filter((r) => r.attempts[0].connection && !r.attempts[0].help)
            .length,
          freshPunctuation: checks.filter((r) => r.attempts[0].punctuation && !r.attempts[0].help)
            .length,
          ...
          : session.lessonId === "repairs"
            ...
            : session.lessonId === "endings"
              ? {
                  ...
                  freshKinds: [
                    ...new Set(checks.filter(independent).map((r) => r.attempts[0].kind)),
                  ],
                }
              : {
...
export function next(session, { skip = false } = {}) {
  ...
  if (record.demonstrated && phase !== "apply") {
    if (session.cursors.simpler >= 2) {
      finish(
        session,
        session.lessonId === "joining"
          ? "You worked through examples with support. Try a short practice later, or ask for help choosing a connection and checking whether both parts can stand alone."
          : session.lessonId === "openings"
  ...
  if (phase === "practice") {
    ...
    const ready =
      session.lessonId === "joining"
        ? Object.keys(joining.kindNames).every((kind) => results.freshKinds.includes(kind))
        : session.lessonId === "openings"
