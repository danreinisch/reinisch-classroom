...
`;
  ...
  function renderSessionOutcomeForm(goal, cardEl, saveIndicatorEl, periodLabel, preRecorded) {
    const config = goal.observation_config || {};
    ...
    if (preRecorded) {
      ...
      badge.className = 'obs-already-recorded';
      badge.innerHTML = OBS_MET_SVG + ' Already recorded today';
    ...
    buttons.forEach(({ label, response, svg }) => {
      ...
      btn.addEventListener('click', async () => {
        ...
        const noteText = container.querySelector('.obs-note-input')?.value || '';
        await saveObservation(goal, { response }, noteText, periodLabel, saveIndicatorEl);
      });
      row.appendChild(btn);
    });
    ...
    noteInput.addEventListener('change', async () => {
      if (selectedResponse) {
        await saveObservation(goal, { response: selectedResponse }, noteInput.value, periodLabel, saveIndicatorEl);
      }
    });
  ...
  function renderTallyForm(goal, cardEl, saveIndicatorEl, periodLabel, preRecorded) {
    const container = document.createElement('div');
    ...
    const updateTally = async () => {
      ...
      if (o > 0) {
        ...
        const noteText = container.querySelector('.obs-note-input')?.value || '';
        await saveObservation(goal, { successful: s, opportunities: o }, noteText, periodLabel, saveIndicatorEl);
      } else {
    ...
    container.appendChild(noOppLink);
    ...
    container.appendChild(noOppBtns);
    ...
    noteInput.addEventListener('change', async () => {
      ...
      const o = Number(oppInput.value) || 0;
      if (noOppResponse) {
        await saveObservation(goal, { response: noOppResponse }, noteInput.value, periodLabel, saveIndicatorEl);
      } else if (o > 0) {
        await saveObservation(goal, { successful: s, opportunities: o }, noteInput.value, periodLabel, saveIndicatorEl);
      }
    });
    container.appendChild(noteInput);
  ...
  function renderPromptCountForm(goal, cardEl, saveIndicatorEl, periodLabel, preRecorded) {
    const config = goal.observation_config || {};
    ...
    [0, 1, 2, 3, '4+'].forEach(val => {
      ...
      btn.addEventListener('click', async () => {
        ...
        const noteText = container.querySelector('.obs-note-input')?.value || '';
        await saveObservation(goal, { promptCount: numVal }, noteText, periodLabel, saveIndicatorEl);
      });
      row.appendChild(btn);
    });
    container.appendChild(row);
    ...
    noteInput.addEventListener('change', async () => {
      if (selectedCount !== null) {
        await saveObservation(goal, { promptCount: selectedCount }, noteInput.value, periodLabel, saveIndicatorEl);
      }
    });
    container.appendChild(noteInput);
  ...
  function renderBehaviorChecklistForm(goal, cardEl, saveIndicatorEl, periodLabel, preRecorded) {
    const config = goal.observation_config || {};
    ...
    container.appendChild(notAddressedBtn);
  ...
  function buildPopupContent(periodLabel, goalsForPeriod, studentsMap, overlayEl) {
    ...
    for (const [studentCode, goals] of byStudent) {
      ...
      for (const goal of goals) {
        ...
        if (category === 'session_outcome') {
          renderSessionOutcomeForm(goal, card, saveIndicatorEl, periodLabel, isRecorded);
        } else if (category === 'tally') {
          renderTallyForm(goal, card, saveIndicatorEl, periodLabel, isRecorded);
        } else if (category === 'prompt_count') {
          renderPromptCountForm(goal, card, saveIndicatorEl, periodLabel, isRecorded);
        } else if (category === 'behavior_checklist') {
          renderBehaviorChecklistForm(goal, card, saveIndicatorEl, periodLabel, isRecorded);
        }
  ...
  function showPopup(periodInfo) {
    ...
    if (countdownEl) countdownEl.textContent = fmtSeconds(remaining);
