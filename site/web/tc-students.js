(async () => {
  ...
  async function renderExpandedDetail(studentCode) {
    ...
    // === Tabs — built with DOM APIs, no innerHTML ===
    const tabsDiv = document.createElement('div');
    tabsDiv.className = 'st-tabs';

    const tabNames = ['goals', 'classes', 'compliance', 'settings'];
    const tabLabels = ['Goals', 'Classes', 'Compliance', 'Settings'];
    tabNames.forEach((name, i) => {
      const btn = document.createElement('button');
      btn.className = 'st-tab' + (selectedDetailTab === name ? ' active' : '');
    ...
    // tabContent is trusted HTML from internal render functions that already use escapeHtml()
    contentDiv.insertAdjacentHTML('afterbegin', tabContent);
  ...
  async function handleSaveProgressData(goalId, e) {
    ...
    try {
      if (goal.measurement_type === 'Accuracy' || goal.measurement_type === 'Percent') {
        ...
        const timePeriod = card.querySelector('[name="time_period"]')?.value || 'per session';
        if (isNaN(count)) {
