...
- `npm run test:dictation:ui`: 14 Chromium tests run the actual Student Portal using synthetic sessions and mocked endpoints. They verify save/reopen/edit/submit, short-answer payloads, Writing Builder transfer/autosave, read-aloud interaction, submission timing, day/close cleanup, submitted/graded work, helper loading failure, permission denial, both HTML embedding paths and their existing bridge, keyboard operation, and a 390 px screen.
...
Speech recognition was simulated in automated tests: these checks establish text handling and portal integration, not the accuracy or availability of a real microphone/service on school-managed devices. Before classroom rollout, try a short synthetic response on a classroom device, including granting/denying permission and reviewing the recognized words.
