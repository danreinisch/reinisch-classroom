...
const local = {
  ...
  async listStudentsWithCounts(filter = {}) {
    ...
    return filtered.map((s) => {
      ...
      const studentEnrollments = enrollments.filter((e) => e.student_code === s.code && e.active);
      return {
        ...s,
        goals_count: goals.length,
  ...
  async updateStudentEnrollments({ code, add = [], remove = [] }) {
    console.log("[student-manager] updateStudentEnrollments (local) - not fully implemented", {
      code,
      add,
      remove,
    });
  ...
  async replaceGoalVersion({ old_goal_id, new_goal }) {
    console.log("[student-manager] replaceGoalVersion (local) - not fully implemented", {
      old_goal_id,
      new_goal,
    });
  ...
  async setStudentActive({ code, active }) {
    console.log("[student-manager] setStudentActive (local) - not fully implemented", {
      code,
      active,
    });
    const students = store.get("students", []);
  ...
  async getStudentEnrollments(student_code) {
    console.log(
      "[student-manager] getStudentEnrollments (local) - not fully implemented",
      student_code
    );
  ...
  async bulkUpdateClassEnrollments(class_id, { addCodes = [], removeCodes = [] }, opts = {}) {
    ...
    for (const code of addCodes) {
      ...
      if (student) {
        const existing = enrollments.find(
          (e) => e.class_id === class_id && e.student_code === code
        );
        if (existing) {
    ...
    for (const code of removeCodes) {
      const idx = enrollments.findIndex((e) => e.class_id === class_id && e.student_code === code);
      if (idx >= 0) {
        enrollments[idx].active = false;
      }
  ...
  async ensureAssignmentInstancesForClass(class_id, studentCodes = []) {
    ...
    const classAssignments = assignments.filter(
      (a) => a.meta?.class_id === class_id || a.class_id === class_id
    );
    ...
    for (const code of studentCodes) {
      ...
      for (const assignment of classAssignments) {
        const existing = instances.find(
          (i) => i.assignment_id === assignment.id && i.student_code === code
        );
        if (!existing) {
          instances.push({
  ...
  async upsertAssignmentGoalMapping({
    assignment_id,
    goal_code,
    student_code,
    primary_goal = false,
  }) {
    console.log("[progress-mapping] upsertAssignmentGoalMapping (local mode)", {
      assignment_id,
      goal_code,
      student_code,
      primary_goal,
  ...
  async deleteAssignmentGoalMapping({ assignment_id, goal_code, student_code }) {
    console.log("[progress-mapping] deleteAssignmentGoalMapping (local mode)", {
      assignment_id,
      goal_code,
      student_code,
    });
  ...
  async recordProgressForSubmission(instance_id) {
    ...
    return {
      success: true,
      inserted_count: 0,
      note: "Local mode stub - use remote for automation",
    };
  },
...
const remote = {
  ...
  async upsertStudent(studentData) {
    ...
    return await withRetry(async () => {
      ...
      if (primary_case_manager !== undefined)
        fullPayload.primary_case_manager = primary_case_manager;
      if (archived_at !== undefined) fullPayload.archived_at = archived_at;
      if (active !== undefined) fullPayload.active = active;

      let { data, error } = await supabase
        .from("students")
        .upsert(fullPayload, { onConflict: "code" })
        .select()
        .single();
      ...
      if (isSchemaError(error)) {
        ...
        const fallback = await supabase
          .from("students")
          .upsert(basicPayload, { onConflict: "code" })
          .select()
          .single();
  ...
  async createStudentWithEnrollmentsAndGoals(payload) {
    ...
    return await withRetry(async () => {
      ...
      const { data, error } = await supabase.rpc("create_student_with_enrollments_and_goals", {
        payload,
      });
      if (error) throw error;
      return data;
  ...
  async getDistinctGoalAreas(student_code = null) {
    ...
    if (!supabase) throw new Error("supabase-not-configured");
    return await withRetry(async () => {
      let query = supabase.from("goals").select("goal_area");
    ...
    return await withRetry(async () => {
      ...
      for (const student of students) {
        for (const assignment of assignments) {
          ...
          const { error: instanceError } = await supabase.from("assignment_instances").upsert(
            {
              assignment_id: assignment.id,
              student_id: student.id,
              status: "Assigned",
  ...
  async listGoalQuarterAverages({ goalIds, studentIds, year } = {}) {
    ...
    return await withRetry(async () => {
      console.log("[goal-progress] listGoalQuarterAverages (remote)", {
        goalIds,
        studentIds,
        year,
      });
  ...
  async bulkInsertGoalProgress(rows = []) {
    ...
    return await withRetry(async () => {
      ...
      // Insert all rows
      const { data, error } = await supabase.from("goal_progress").insert(insertRows).select();
  ...
  async upsertAssignmentGoalMapping({ assignment_id, goal_id, primary_goal = false }) {
    ...
    return await withRetry(async () => {
      console.log("[progress-mapping] upsertAssignmentGoalMapping (remote)", {
        assignment_id,
        goal_id,
        primary_goal,
      });
  ...
  async deleteAssignmentGoalMapping({ assignment_id, goal_id }) {
    ...
    return await withRetry(async () => {
      console.log("[progress-mapping] deleteAssignmentGoalMapping (remote)", {
        assignment_id,
        goal_id,
      });
  ...
  async recordProgressForSubmission(instance_id) {
    ...
    return await withRetry(async () => {
      ...
      const { data, error } = await supabase.rpc("record_progress_for_submission", {
        p_instance_id: instance_id,
      });
  ...
  async createPortalSavedView(userCode, { name, view_type = "assignments", config }) {
    ...
    return await withRetry(async () => {
      console.log("[portal-saved-views] createPortalSavedView (remote)", {
        userCode,
        name,
        view_type,
      });
