...
// Internal: Called by teacher-ai-skills-summary-submit after it validates auth and inserts the job.
...
// to the caller immediately. The actual OpenAI work happens asynchronously.
//
// Flow:
//   1. Check for a recent cached result (same payload_hash, last 24h)
//   2. If cache hit → update job to 'complete' immediately
//   3. Otherwise → call OpenAI (up to 3 retries with backoff) → update job
//
// Auth and input validation are handled by the synchronous submit gateway
// (teacher-ai-skills-summary-submit.js). This function trusts the payload it receives.
...
// This means switching audience for the same student triggers a new OpenAI call, which is
// intentional — the summaries have different tone, terminology, and structure.
...
exports.handler = async (event) => {
  ...
  const { job_id, student_code, iep_goals, dese_standards, audience } = parseResult.data;
