// Netlify Function: Incremental Deploy for adding a single presentation/module
// Expects Netlify env vars: NETLIFY_TOKEN, SITE_ID, PUBLIC_SITE_URL
...
function redirectIndexHtml(title, targetRel, backHref, section) {
  ...
  if (!targetRel) {
    return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${title}</title></head><body>${navHtml}<div style="display:grid;place-items:center;height:100vh;color:#e8edf5"><div><h1>${title}</h1><p>No HTML file was found in this presentation folder.</p><p><a href="${backHref}">Back</a></p></div></div></body></html>`;
  }
  return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta http-equiv="refresh" content="0; url=${targetRel}"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${title}</title><script>location.replace(${JSON.stringify(targetRel)});</` + `script></head><body>${navHtml}</body></html>`;
}
...
function generateCategoryIndex(catId, state) {
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>${pageTitle} – Reinisch Classroom</title><style>*{box-sizing:border-box;margin:0;padding:0}:root{--glass:rgba(255,255,255,.14);--glass-brd:rgba(255,255,255,.28);--text:#e8edf5}body{min-height:100vh;font-family:Segoe UI,Roboto,Arial,sans-serif;color:var(--text);background:#0b1220;display:flex;flex-direction:column;align-items:center;text-align:center;padding:2rem}.grid{width:100%;max-width:1100px;display:grid;gap:1rem;grid-template-columns:repeat(auto-fit,minmax(220px,1fr))}.card{background:var(--glass);border:1px solid var(--glass-brd);border-radius:1rem;padding:1rem 1.25rem;color:var(--text);text-decoration:none;min-height:86px;display:flex;flex-direction:column;justify-content:center;align-items:center;box-shadow:0 10px 30px rgba(0,0,0,.15)}</style></head><body><header><h1>${pageTitle}</h1><p style="opacity:.9;margin:14px 0 22px">Unit hub</p></header><section class="grid">${cards}</section></body></html>`;
}
...
exports.handler = async (event) => {
  try {
    ...
    const { category, slot, title, files } = JSON.parse(event.body || '{}');
    if (!CAT_META[category]) return json(400, { message: 'Unknown category' });
    if (!slot || slot < 1 || slot > CAT_META[category].slots) return json(400, { message: 'Invalid slot' });
    if (!title || !files || !Array.isArray(files) || files.length === 0) return json(400, { message: 'Missing title/files' });

    const token = process.env.NETLIFY_TOKEN;
    const siteId = process.env.SITE_ID;
    if (!token || !siteId) return json(500, { message: 'Server not configured (token/site id)' });

    // 1) Get latest deploy for this site
    ...
    // 2) List files from previous deploy (path -> sha)
    ...
    // 5) Determine entry HTML and add redirect index.html
    const slotHtmls = Array.from(newBlobs.keys()).filter(p => p.startsWith('/' + slotDir + '/') && p.toLowerCase().endsWith('.html'));
    let entryRel = null;
    if (slotHtmls.length) {
      slotHtmls.sort((a,b) => {
      ...
      entryRel = slotHtmls[0].replace('/' + slotDir + '/', '');
    }
    ...
    // 6) Update state and regenerate category index
    ...
    // 7) Merge previous manifest with new/updated paths
...
async function listDeployFiles(deployId, token){
  ...
  while(true){
    const res = await apiGET(`https://api.netlify.com/api/v1/deploys/${deployId}/files?per_page=10000&page=${page}`, token);
