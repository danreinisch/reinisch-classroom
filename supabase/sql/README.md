...
### `repair_student_auth_usernames_passwords.sql`

**Purpose:** Fix student authentication issues caused by duplicate app_users rows differing only by case and legacy password policy requiring "!" suffix.

**Usage:**

1. Open the SQL Editor in your Supabase dashboard
2. Copy the contents of `repair_student_auth_usernames_passwords.sql`
3. Execute the script
4. Review the progress notices in the output
5. Uncomment and run the verification queries at the end (Step D)

**Features:**

- Detects and reports case-colliding student usernames (e.g., S004 vs s004)
- Removes lowercase student username duplicates (preserves uppercase canonical versions)
- Resets student passwords to match their code exactly (removes legacy "!" suffix)
- Safe to run multiple times (idempotent)
- Only affects student accounts (role='student')
- Provides detailed verification queries

**Expected Output:**

- Step A: Reports case-colliding username groups
- Step B: Deletes lowercase student username variants
- Step C: Resets passwords for uppercase student codes (S001, S002, etc.)
- Summary: Total collisions, deletions, and password resets

**Notes:**

- Run this script if students cannot authenticate with their code
- After running, students log in with their code only (no "!" suffix)
- See detailed documentation in `docs/STUDENT_AUTH_REPAIR.md`
- Does not modify table structure, constraints, or non-student accounts
