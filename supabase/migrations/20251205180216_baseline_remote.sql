...
CREATE OR REPLACE FUNCTION "public"."verify_user_password"("p_username" "text", "p_password" "text") RETURNS TABLE("username" "text", "role" "text", "student_id" bigint, "user_id" bigint)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public', 'pg_catalog'
    AS $$
DECLARE
...
CREATE TRIGGER on_teacher_metadata_update AFTER UPDATE ON auth.users FOR EACH ROW EXECUTE FUNCTION public.sync_teacher_metadata();

CREATE TRIGGER on_teacher_signup AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.handle_teacher_signup();
...
using ((bucket_id = 'assignments'::text));
  ...
  to public
using ((bucket_id = 'assignments'::text))
with check ((bucket_id = 'assignments'::text));



  create policy "public upload assignments"
  on "storage"."objects"
  as permissive
  for insert
  to public
