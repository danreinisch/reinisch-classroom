...
-- Purpose: Authoritatively correct ALL goal descriptions, goal_area, and case_manager
--          in the goals table to match the master IEP CSV (source of truth).
--          Also purges all test data from goal_progress, submissions,
--          submission_answers, and resets assignment_instance statuses.
--
-- Background: Goal descriptions in the database are misaligned with the CSV source
-- of truth across the entire student caseload — not just S001. The desc column has
-- wrong text paired with certain goal codes as a result of the original CSV import.
-- This migration is the authoritative one-time reconciliation fix.
--
...
--                  explicit corrections in the problem statement for S001–S005.
...
-- ============================================================
-- STEP 2: Correct all goal descriptions to match master CSV.
...
-- S006.11.1 — Behavior
-- NOTE: "duration's" is verbatim from the IEP document and must not be silently corrected here.
UPDATE public.goals g
...
-- S009.11.4-1 — Written Expression
UPDATE public.goals g
SET "desc"        = 'S009-Writing: Topic/Claim',
    goal_area     = 'Written Expression',
    case_manager  = 'Sara Koelsch'
FROM public.students s
WHERE g.student_id = s.id
...
-- S009.11.4-2 — Written Expression
UPDATE public.goals g
...
-- S009.11.4-3 — Written Expression
UPDATE public.goals g
...
-- S012.12.6 — Life Skills Transition
UPDATE public.goals g
...
-- S016.11.3-1 — Written Expression
UPDATE public.goals g
...
-- S016.11.3-2 — Written Expression
UPDATE public.goals g
...
-- S016.11.3-3 — Written Expression
UPDATE public.goals g
...
-- S017.9.1 — Basic Reading
-- NOTE: "the life if the IEP cycle" is verbatim from the IEP document (typo in original).
UPDATE public.goals g
...
SET "desc"        = 'S024 will increase his math problem solving skills by solving multiple step problems involving the use of decimals with 70% accuracy by the next annual IEP review date.',
    goal_area     = 'Math Problem Solving',
    case_manager  = 'Dan Reinisch'
FROM public.students s
WHERE g.student_id = s.id
  AND s.code = 'S024'
...
-- NOTE: "the life life of this IEP" is verbatim from the IEP document (duplicate word in original).
UPDATE public.goals g
...
-- NOTE: "inference's" is verbatim from the IEP document and must not be silently corrected here.
UPDATE public.goals g
...
-- S033.10.2 — Reading Comprehension
UPDATE public.goals g
...
SET "desc"        = 'S043 will maintain her emotional regulation skills by using learned strategies to calm her anxiety, work independently, and complete assignments 80% of the time on data collection days.',
    goal_area     = 'Emotional Regulation',
    case_manager  = 'Kathie Tate'
FROM public.students s
WHERE g.student_id = s.id
  AND s.code = 'S043'
...
-- NOTE: "Kathy Tate" is verbatim from the CSV for this student (other students use "Kathie Tate").
UPDATE public.goals g
...
UPDATE public.goals g
SET "desc"        = 'S045 will increase his written expression skills by restating questions and adding 3 supporting details to his writing with 50% accuracy by the end of the annual IEP cycle. Baseline: 35%',
    goal_area     = 'Written Expression',
    case_manager  = 'Jessica Bruno'
FROM public.students s
WHERE g.student_id = s.id
...
-- NOTE: This row was malformed in the CSV (multiline text split across rows).
--       The full description is reconstructed here.
UPDATE public.goals g
