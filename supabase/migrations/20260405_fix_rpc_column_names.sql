-- Migration: 20260405_fix_rpc_column_names
-- Purpose:   Rebuild all three goal-management RPC functions so that they
--            1. Use the correct goals table column names:
--               • `code`   instead of `goal_code`
--               • `"desc"` instead of `goal_text`
--            2. Include every enriched column in INSERT statements:
--               class_context, data_collector, data_collector_email,
--               measurement_type, notes, observation_config
--            3. For replace_goal_version: carry forward ALL enriched fields
--               from the old goal record using COALESCE(new_value, v_old.field)
--            4. Always set student_id on every new goal row
--
-- This migration is idempotent — it uses CREATE OR REPLACE FUNCTION throughout.
--
-- JSON input keys
-- ---------------
-- For backward-compatibility the functions accept both old-style keys
-- (goal_code / goal_text) and current-style keys (code / desc) in the input
-- JSONB.  COALESCE is used to prefer the current-style key when both are
-- present.
