-- Migration: 20260405_goal_delivery_fields
-- Purpose:   Add two new boolean columns to the goals table to track how
--            each goal is delivered, so the AI Builder can include or skip
--            goals appropriately when generating class vs. individual work.
--
--   addressed_in_class  boolean NOT NULL DEFAULT true
--     When true  → goal is targeted in the teacher's class assignment.
--     When false → skip this goal for class-scoped assignments.
--
--   individual_delivery  boolean NOT NULL DEFAULT false
--     When true  → goal is addressed through individual work (Data Probe,
--                  pull-out, 1:1 session).  AI Builder prioritises it for
--                  Data Probe generation.
--
-- The migration is idempotent — ADD COLUMN IF NOT EXISTS is used throughout.

ALTER TABLE public.goals
  ADD COLUMN IF NOT EXISTS addressed_in_class boolean NOT NULL DEFAULT true;

ALTER TABLE public.goals
  ADD COLUMN IF NOT EXISTS individual_delivery boolean NOT NULL DEFAULT false;

-- ============================================================================
-- Rebuild RPC functions to carry the new fields
-- ============================================================================
